...
需要方法区别 未存档、已存档的数据
idletime不适合做这个事情，因为这个数据只能说明数据变化情况，并不能说明是否已经存档。
一个方案就是用某个数据结构把已存档的key保存起来，使用set

需要方法区别 已存档的数据里，哪些不被访问到的概率更高
可以查询一个key的lur活跃情况，然后把较不活跃的那部分key回收

unix socket是否比network socket快？
